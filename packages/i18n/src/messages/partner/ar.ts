...
export const ar = {
  ...
  coupons: {
    ...
    warning:
      'القبول نهائي. بعد قبولك يبقى الكوبون فعّالاً على عقاراتك حتى تاريخ انتهاء صلاحيته، ولا يمكن التراجع عن القرار أو سحبه.',
    confirmTitle: 'قبول الكوبون',
    confirmBody:
      'القبول نهائي. بعد قبولك يبقى الكوبون فعّالاً على عقاراتك حتى تاريخ انتهاء صلاحيته، ولا يمكن التراجع عن القرار أو سحبه. هل تريد المتابعة؟',
  ...
  dashboard: {
    ...
    */
    ...
    /*
      المهلة والغرامة من الإعدادات لا من النص (باشار، 2026-09-07).

      كانت «مهلة ساعتين — الغرامة 10$» مكتوبة حرفياً فوق طابور الطلبات: القاعدة التي يُحاسَب عليها
      الشريك، على الشاشة التي يكلّفه الخطأ فيها مالاً. تغيير المهلة إلى تسعين دقيقة كان يترك اللافتة
      تَعِد بساعتين.
    */
    requestsRule: 'مهلة {window} — الغرامة {fine} عند عدم الرد',
    /*
      المدة تمر بـ ICU لأن «ساعتين» مثنّى: صحيحة عند 120 دقيقة وخاطئة عند كل قيمة أخرى، وللعربية ست
      صيغ جمع. الصفر لا يظهر عملياً — المهلة لا تقل عن دقيقة — لكن صيغة له أفضل من «0 ساعة».
    */
    durationHours:
    ...
    reject: 'رفض',
    /*
      القبول يمر بنافذة تسمّي الحجز الذي يجري قبوله (باشار، 2026-09-07).

      كان زر «قبول» ينقل إلى حالة زرّها الأساسي «قبول» أيضًا: الخطوتان متطابقتان في النص واللون،
      والفرق الوحيد أن «رفض» يصير «إلغاء». الضغطة الأولى لا ترسل شيئًا — فمن يضغط مرة واحدة ثم
      ينصرف لم يقبل الحجز، وأمامه ساعتان وغرامة 10$.

      لذلك: عبارة تأكيد مختلفة عن عبارة الزر، والنافذة تذكر المرجع والوحدة والتواريخ حتى لا يُقبل
      حجز غير المقصود من قائمة يجري مسحها بسرعة.
    */
    acceptTitle: 'تأكيد قبول الحجز',
    acceptBody: '{reference} · {unit} · {checkIn} ← {checkOut} · {amount}',
    acceptWarning: 'بالقبول تلتزم باستضافة الضيف، ولا يمكن التراجع.',
    acceptConfirm: 'نعم، أقبل الحجز',
    /*
      وبعد القرار: الصف يقول ما حدث بدل أن يختفي.

      كان `router.refresh()` يزيل الطلب من القائمة فورًا، و«اختفى» تقرأ أيضًا «انتهت المهلة».
      باشار: «المستخدم يجب أن يعرف دائمًا إن كان الحجز قد قُبل فعلًا أم لا».
    */
    accepted: 'تم قبول الحجز',
    acceptedNote: 'أُبلغ الضيف. تجد الحجز في صفحة الحجوزات.',
    rejectedDone: 'تم رفض الطلب',
    rejectedNote: 'أُبلغ الضيف وسفرة بالسبب الذي كتبته.',
  ...
  */
  ...
  editProperty: {
    ...
    */
    ...
    unitBeds: 'الأسرّة',
    /*
      نوع السرير، إلى جانب عددها (بشار، 2026-09-16): «كل سرير يجب أن يُعرَّف سريرًا مزدوجًا أو
      فرديًا، وكذلك عند تعدّد الأسرّة». لا خيار ثالث ولا خانة فارغة — لأن المنصّة لا تملك عبارة
      تكتب بها سريرًا بلا نوع.
    */
    unitBedType: 'نوع السرير',
    unitBedTypeSingle: 'سرير فردي',
    unitBedTypeDouble: 'سرير مزدوج',
  ...
  */
  ...
  payouts: {
    ...
    withheldNote:
      /*
        «لا يلزمك أي إجراء» كانت صحيحة، وصارت خاطئة يوم صار للشريك صوت (اكتشاف 223).

        قبل ذلك كانت سفرة تبتّ وتُعلم الشريك بالنتيجة فحسب، فلم يكن أمامه ما يفعله. الآن
        يُقرأ ردّه قبل القرار، والنزاع الذي لم يردّ عليه ينتظره هو — وشاشةٌ تقول له إنه لا
        يلزمه شيء تُبقيه ساكتًا ثم تفاجئه بقرارٍ اتُّخذ دون روايته. الشرط الدقيق مكتوب على
        كل صف؛ هذه الجملة تقول القاعدة العامة ولا تَعِد بما لم يعد صحيحًا.
      */
      'على هذه الحجوزات نزاعات مفتوحة، فلا تُحوَّل مستحقاتها حتى تُغلق. تُقرأ روايتك قبل البتّ في كل نزاع، وما ينتظر ردّك موضَّح أدناه.',
    /*
    ...
    */
    ...
    /*
      ── ما يحتاجه الشريك ليفهم التجميد (باشار، 2026-09-08) ───────────────────

      «أريد أن يفهم الشريك: أي الحجوزات متأثرة، وأي النزاعات مسؤولة، وكم المبلغ المجمّد، ولماذا
      جُمّد، وما الحدث الذي يجب أن يقع قبل الإفراج عنه.»

      الثلاثة الأولى كانت موجودة. أما «لماذا» فكانت رقم نزاع يحتاج بحثًا، و«ما الحدث» جملةً عامة
      أسفل الصفحة. الآن: نوع الشكوى، وحالتها، وشرط الإفراج مكتوبًا لكل صف — ومع الشرط الفعل، إن
      كان الفعل للشريك.
    */
    withheldWhy: 'سبب التجميد',
    withheldRelease: 'يُفرج عن المبلغ عند إغلاق النزاع.',
    withheldReleaseNeedsYou:
      'لم يصل ردّك بعد. لا يُبتّ في النزاع قبل قراءة روايتك — اكتبها الآن ليمضي القرار.',
    withheldOpen: 'فتح النزاع',
    withheldResponded:
      '{count, plural, one {ردّك مسجَّل} two {ردّان مسجَّلان} few {# ردود مسجَّلة} many {# ردًّا مسجَّلًا} other {# ردّ مسجَّل}}. ننتظر قرار فريق سفرة.',
  ...
  */
  ...
  disputes: {
    ...
    /*
      ── رفع الشريك لأدلته (باشار، 2026-09-08) ───────────────────────────────

      «يجب أن يستطيع الشريك رفع أدلة ومستندات مؤيّدة.» شكوى مثل «الغرفة لا تطابق الصور المنشورة»
      تُحسم بمقارنة صور، ولم يكن أحد الطرفين قادرًا على تقديم أي منها.
    */
    evidenceAdd: 'أضف صورة',
    evidenceUploading: 'جارٍ الرفع…',
    evidenceProcessing: 'جارٍ المعالجة…',
    evidenceSlow: 'لم تكتمل المعالجة بعد',
    evidenceHint:
      'أضف صورًا تدعم روايتك — الغرفة، أو الوحدة، أو ما يُظهر ما حدث. تُقرأ مع ردّك قبل البتّ وتبقى في سجل النزاع. الحد الأقصى 10 ميغابايت للصورة الواحدة.',
    evidenceClosedNote: 'أُغلق النزاع، ولم يعد ملفّه يقبل إضافات.',
    evidenceOpen: 'معاينة',
  ...
  violations: {
    ...
    */
    defaultDescription: {
      /*
        «خلال المهلة المتاحة» وليس «خلال مهلة الساعتين»، ولا حتى المهلة الحالية من الإعدادات.

        هذه الجملة تصف حدثاً وقع في الماضي، وقد حُوكم بالمهلة السارية حينها. لو وُضعت القيمة الحالية
        لصارت مخالفة سُجّلت في آذار موصوفة بمهلة اليوم — كذبة جديدة مكان القديمة. المهلة الفعلية لذلك
        الحجز ظاهرة على الحجز نفسه.
      */
      no_response:
        'لم يصل ردّ على طلب الحجز {reference} خلال المهلة المتاحة للرد، فأُلغي الطلب تلقائياً وسُجّلت المخالفة.',
    ...
    defaultDescriptionNoBooking: {
      no_response:
        'لم يصل ردّ على طلب الحجز خلال المهلة المتاحة للرد، فأُلغي الطلب تلقائياً وسُجّلت المخالفة.',
      rejected_after_payment:
