...
export const ar = {
  ...
  admin: {
    ...
    */
    ...
    /*
      النسبة والرسوم من الإعدادات لا من النص (باشار، 2026-09-07).

      «عمولة الشريك 7٪ + رسوم خدمة 1.99$» كانت مكتوبة حرفياً على بطاقة في التطبيق نفسه الذي يُحرَّر
      منه الإعدادان. تغيير الرسوم من هذه الشاشة كان يترك بطاقتين تقتبسان القيمة القديمة.
    */
    weekRevenueSub: 'عمولة الشريك {rate}٪ + {fee} · بالدولار',
    /* الرسوم إمّا مبلغ ثابت وإمّا نسبة — والوضعان إعدادان، فلكلٍّ جملته. */
    feeFlat: 'رسوم خدمة {value}',
    /* «ساعتين» مثنّى: صحيح عند 120 دقيقة فقط، وللعربية ست صيغ جمع — فالمدة تمر بـ ICU. */
    durationHours:
      /*
        المثنّى بصيغة الجر «ساعتين» لا الرفع «ساعتان»: كل جملة تضع القيمة بعد حرف جر («خلال»، «بعد»)،
        وهذه صياغة مقصودة — رسالة ICU واحدة لا يمكن أن تصحّ في موضعَي الرفع والجر معاً، فوُحِّد الموضع.
      */
      '{count, plural, zero {أقل من ساعة} one {ساعة} two {ساعتين} few {# ساعات} many {# ساعة} other {# ساعة}}',
    durationMinutes:
      '{count, plural, zero {أقل من دقيقة} one {دقيقة} two {دقيقتين} few {# دقائق} many {# دقيقة} other {# دقيقة}}',
    feePercent: 'رسوم خدمة {value}٪ من قيمة الإقامة',
  ...
  sections: {
    ...
    */
    ...
    wallet: {
      ...
      */
      ...
      /*
        بلا رقم: «مثل 10$» كانت نسخة من `wallet.sla_compensation`، والجدول فوق هذه الملاحظة يعرض
        المبالغ الفعلية لكل حركة. مثال برقم ثابت يصبح خاطئاً يوم تتغيّر القيمة، والوصف بلا رقم صحيح
        دائماً ولا يفقد القارئ شيئاً.
      */
      note: 'المحفظة تخزن تعويضات العملاء (P-007) — كالتعويض المستحق عند فشل الشريك بالرد أول مرة — وتُستخدم مع بطاقة هدية ووسيلة دفع أخرى في عملية واحدة. كل حركة لها سبب مسجَّل.',
    },
    ...
    geo: {
      ...
      currencyNote:
        'الدولار الأمريكي هو عملة المنصة الوحيدة: تُعرض به الأسعار وتُحصَّل به المدفوعات.',
      citiesNote:
    ...
    reports: {
      ...
      commissionRevenueSub: '{fee} للعميل + عمولة {rate}٪ شريك · هذا الأسبوع',
      ...
      /* المهلة من الإعدادات: «ساعتان» صحيحة عند 120 دقيقة وخاطئة عند كل قيمة أخرى. */
      partnerResponseSub: 'يُلغى الطلب وتُسجَّل الغرامة بعد {window} دون رد',
      minutes: 'دقيقة',
    ...
    */
    ...
    disputes: {
      ...
      */
      ...
      /* الطرف الثالث: الشريك يرفع أدلته منذ اكتشاف 223، ولم يكن ذلك ممكنًا قبله. */
      evidenceFiledByPartner: 'قدّمه الشريك',
      /*
        مشاركة ملف من أدلة العميل مع الشريك — قرار صريح لا سلوك افتراضي.

        باشار، 2026-09-08: «إن قرّر الفريق أن ملفًا من العميل ضروري لبتٍّ عادل، فتلك خطوة صريحة من
        الفريق وليست السلوك الافتراضي.» فالزر موجود على كل ملف من العميل، والافتراضي أنه غير مُشارَك.
      */
      evidenceShare: 'شارِك',
      evidenceUnshare: 'حجب',
      /* على الصورة نفسها: «من يرى هذا؟» صفةٌ للصورة، لا سطرٌ في قائمة. */
      evidenceSharedBadge: 'مرئي للشريك',
      evidenceSharing: 'جارٍ…',
      evidenceShareTitle: 'مشاركة هذا الملف مع الشريك؟',
      /*
        نصٌّ صادق: المشاركة قابلة للتراجع، والتراجع يُسجَّل أيضًا. النسخة الأولى قالت «لا يمكن
        التراجع عنها» وكان ذلك غير صحيح — الخدمة تدعم الحجب — وشاشةٌ تقول ما ليس صحيحًا أسوأ من
        شاشةٍ ناقصة.
      */
      evidenceShareMessage:
        'يصبح هذا الملف مرئيًّا للشريك داخل صفحة النزاع. شارِكه فقط إذا كان ضروريًّا لبتٍّ عادل — أدلة العميل خاصة افتراضيًّا. تُسجَّل المشاركة في سجل التدقيق باسمك، ويمكنك حجب الملف لاحقًا ويُسجَّل الحجب كذلك.',
      evidenceShareConfirm: 'نعم، شارِك الملف',
      evidenceUnshareTitle: 'حجب هذا الملف عن الشريك؟',
      evidenceUnshareMessage:
        'لن يعود الشريك قادرًا على فتح هذا الملف. يبقى في سجل التدقيق أنه كان مُشارَكًا ومتى، فالحجب إضافةٌ إلى السجل لا محوٌ منه.',
      evidenceUnshareConfirm: 'نعم، احجب الملف',
      evidenceShareFailed: 'تعذّرت العملية. حاول مرة أخرى.',
      evidenceShareNote:
        'أدلة العميل خاصة افتراضيًّا ولا يراها الشريك. شارِك ملفًا فقط إذا كان ضروريًّا لبتٍّ عادل.',
      evidenceFiledByCustomer: 'من العميل',
      evidenceUploading: 'جارٍ الرفع…',
      evidenceProcessing: 'جارٍ المعالجة…',
      evidenceSlow: 'لم تكتمل بعد',
      /*
        ── ردّ الشريك على الشكوى (باشار، 2026-09-08) ───────────────────────────

        «لا أريد أن تبتّ سفرة في النزاعات وطرفٌ واحد فقط له صوت في العملية.» صار للشريك صوت، وردٌّ
        لا يقرؤه من يبتّ يساوي لا ردّ — فمكانه الشاشة التي يُغلق فيها النزاع، بجانب الشكوى التي
        يجيب عنها.
      */
      partnerResponses: 'ردّ الشريك',
      partnerResponsesNone: 'لم يصل ردّ من الشريك بعد. القرار يُتخذ بعد قراءة ردّه.',
      acknowledge: 'استلام',
    ...
    comms: {
      ...
      /* ── إعادة الإرسال (Bashar, 2026-09-09) ──────────────────────────────────────────────── */
      redrive: 'إعادة الإرسال',
      redriveQueued: 'أُعيدت إلى الطابور',
      redriveTitle: 'إعادة إرسال الإشعار',
      redriveAsk:
        'سيُعاد وضع هذا الإشعار في طابور الإرسال. لن تُرسل نسخة ثانية إذا كان قد وصل أصلاً.',
      redriveAbandonedAsk:
        'استُنفدت محاولات هذا الإشعار ولن يُعاد إرساله تلقائياً. سيُعاد وضعه في الطابور الآن بمحاولة إضافية.',
      redriveConfirm: 'أعد الإرسال',
      redriveUnreconstructable:
        'تعذّر إعادة بناء هذه الرسالة: لم يعد المستلم أو السجل المرتبط بها متاحاً.',
      redriveHint:
        'الإشعارات التي فشلت تُعاد تلقائياً بعد انتهاء مهلة المحاولات. «توقّفت المحاولات» تحتاج قراراً بشرياً.',
    },
    ...
    */
    ...
    emergency: {
      ...
      /* «الساعتين» كانت نسخة من إعداد؛ الزر يسمّي ما يفعله، والمدة تُقرأ حيث تُعرض القيمة. */
      suspendSla: 'تعليق مهلة تأكيد الحجز',
  ...
  enums: {
    ...
    */
    ...
    payloadKey: {
      ...
      /*
        ── إشعارات النزاع ومشاركة أدلته (اكتشاف 223) ────────────────────────

        `event` يقول أي إشعار أُرسل للشريك: فتح النزاع أم بدء مراجعته. كان `dispute.notified`
        يحمل النتيجة فقط، ولم يكن هناك ما يُرسل قبل الإغلاق.

        `filedBy` بديل `byStaff` — صار للأدلة ثلاثة مصادر لا اثنان، وصفٌ منطقيّ ثنائي كان
        سينسب صورة الشريك إلى فريق سفرة.

        `sharedWithPartner` هو القرار الصريح بإظهار ملف من العميل للشريك، وهو ما يجعل الحجب
        هو الأصل والمشاركة هي الاستثناء.
      */
      event: 'الحدث',
      filedBy: 'مَن قدّمه',
      sharedWithPartner: 'مُشارَك مع الشريك',
      ...
      */
      ...
      descriptionEn: 'الوصف بالإنجليزية',
      descriptionDe: 'الوصف بالألمانية',
      headlineAr: 'الجملة التعريفية بالعربية',
      headlineEn: 'الجملة التعريفية بالإنجليزية',
      headlineDe: 'الجملة التعريفية بالألمانية',
      tagsAr: 'الوسوم بالعربية',
      tagsEn: 'الوسوم بالإنجليزية',
      tagsDe: 'الوسوم بالألمانية',
    ...
    */
    ...
    payloadValue: {
      ...
      /* حدث إشعار النزاع: الفتح، مقابل «under_review» أعلاه. */
      opened: 'فتح النزاع',

      /*
        نتيجة النزاع وحالته بعد الإغلاق. الكلمة نفسها التي يقرؤها سجل النزاعات
        (`enums.disputeStatus.resolved`) كي لا يرى القارئ اسمين لقرار واحد. لا تظهر هذه
      ...
      */
      ...
      /*
        ...
        `under_review` فهو اسم الحدث لا اسم الحالة — والحالة نفسها تُقرأ «قيد المراجعة» في
        `enums.disputeStatus`، فالكلمة واحدة عن قصد كي لا يرى القارئ حدثين لحالة واحدة.
      */
      under_review: 'بدء مراجعة النزاع',
    ...
    */
    violationDefaultDescription: {
      /*
        «خلال المهلة المتاحة» لا «مهلة الساعتين»، ولا المهلة الحالية من الإعدادات: الجملة تصف حدثاً
        ماضياً حُوكم بالمهلة السارية حينها، ووضع قيمة اليوم يجعل مخالفة قديمة موصوفة بقاعدة لم تكن
        سارية. المهلة الفعلية لذلك الحجز على الحجز نفسه.
      */
      no_response:
        'لم يرد الشريك على طلب الحجز {reference} خلال المهلة المتاحة للرد، فأُلغي الطلب تلقائياً وسُجّلت المخالفة.',
    ...
    violationDefaultDescriptionNoBooking: {
      no_response:
        'لم يرد الشريك على طلب حجز خلال المهلة المتاحة للرد، فأُلغي الطلب تلقائياً وسُجّلت المخالفة.',
      rejected_after_payment:
    ...
    */
    ...
    notificationStatus: {
      ...
      sent: 'مرسلة',
      delivered: 'وصلت',
      failed: 'فشلت — إعادة تلقائية',
      abandoned: 'توقّفت المحاولات',
    } as Record<string, string>,
  ...
  notificationTemplate: {
    ...
    */
    ...
    'dispute.payout_released': 'قرار النزاع ورفع تجميد المستحقات',
    'partner.dispute_opened': 'إبلاغ الشريك بفتح نزاع',
    'partner.dispute_under_review': 'إبلاغ الشريك ببدء مراجعة النزاع',
