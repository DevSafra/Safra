  ...
  return (
    <main className="mx-auto grid max-w-3xl gap-10 px-4 py-10">
      <header>
        ...
        <p className="mt-2 text-16 leading-relaxed text-text">{t('subtitle')}</p>
        <p className="mt-3 text-16 leading-relaxed text-muted">{t('intro')}</p>
      </header>
      ...
      <section>
        ...
        <ul className="grid gap-2 sm:grid-cols-2">
          {reasons.map((key) => (
            <li
              key={key}
              className="rounded-card border border-line bg-card px-4 py-3 text-14 leading-relaxed text-text2"
            >
              {t(`why.${key}`)}
            </li>
          ))}
      ...
      <section>
        ...
        */}
        <ul className="grid list-inside list-disc gap-2 marker:text-gold-read">
          {documents.map((key) => (
            <li key={key} className="text-14 leading-relaxed text-text2">
              {t(`documents.${key}`)}
            </li>
          ))}
        </ul>
        ...
        */}
        <p className="mt-3 rounded-card border border-dashed border-line px-4 py-3 text-14 leading-relaxed text-faint">
          {t('documentsNote')}
        </p>
      </section>
      ...
      <section>
        ...
        <p className="mb-1 text-14 text-faint">{t('formNote')}</p>
        {/* Why there is no email box, said where somebody would look for one. */}
        <p className="mb-4 text-14 text-faint">{t('signedInNote')}</p>
