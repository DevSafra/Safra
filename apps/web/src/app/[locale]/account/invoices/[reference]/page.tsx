...
import { customerLines } from '@/lib/customer-fee';
...
export default async function AccountInvoicePage({
  ...
  /*
    Whether the fee is NAMED on this document. One setting, read the same way the checkout reads
    it — a receipt that itemises a charge the checkout did not, or the reverse, is two documents
    about one payment that disagree. See `@safra/contracts/customer-fee.ts`.
  */
  const feeVisible = customerFeeVisible(await getPublicSettings());
      ...
      <div className="grid gap-6">
        <div className="flex flex-wrap items-center gap-3 print:hidden">
          ...
          */}
          <a
            ...
            className="inline-flex min-h-10 w-fit cursor-pointer items-center gap-2 rounded-lg border border-gold px-4 text-sm text-gold-read transition-colors hover:btn-gold hover: print:hidden lg:min-h-0 lg:py-2"
          >
        ...
        <section className="rounded-card border border-line bg-card p-5">
          ...
          <Link
            ...
            className="mt-4 inline-flex min-h-10 w-fit items-center text-sm text-gold-read hover:underline print:hidden lg:min-h-0"
          >
        ...
        <section className="rounded-card border border-line bg-card p-5">
          ...
          <dl className="mt-2 divide-y divide-line">
            {customerLines(invoice.lines, invoice.currencyCode, feeVisible).map(
              (line) => (
                <Line
                  key={line.key}
                  line={line}
          ...
          <div className="mt-3 flex items-baseline justify-between gap-4 border-t border-line pt-3">
            ...
            <span className="font-display text-xl text-gold-read" dir="ltr">
