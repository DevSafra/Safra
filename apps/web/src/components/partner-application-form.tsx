  ...
  if (reference) {
    return (
      ...
      >
        ...
        <p className="mt-2 text-16 leading-relaxed text-text">
          {t('successBody', { reference })}
        </p>
        <p className="mt-2 text-14 leading-relaxed text-muted">{t('successKeep')}</p>
        <Link
          href={`/${locale}`}
          className="mt-4 inline-flex min-h-10 items-center rounded-lg border border-line px-4 text-14 text-muted lg:min-h-0 lg:py-2"
        >
          {t('backHome')}
        </Link>
      </section>
  ...
  return (
    <form onSubmit={(event) => void submit(event)} noValidate className="grid gap-5">
      <fieldset className="grid gap-4">
        <legend className="mb-2 text-16 font-bold text-gold-read">
          {t('sectionContact')}
        </legend>
        ...
        */}
        <div className="grid gap-1.5">
          <span className="text-14 font-semibold text-text">{t('email')}</span>
          <p className="min-h-11 rounded-card border border-line bg-card px-3.5 py-3 text-16 text-muted">
            {email}
          </p>
          <span className="text-13 text-faint">{t('emailHint')}</span>
        </div>
      ...
      <fieldset className="grid gap-4">
        ...
        <label className="grid gap-1.5">
          <span className="text-14 font-semibold text-text">{t('message')}</span>
          <textarea
            name="message"
            rows={4}
            maxLength={2000}
            className="rounded-card border border-line bg-field px-3.5 py-2.5 text-16 text-text"
          />
        </label>
      </fieldset>
      ...
      <p className="text-13 leading-relaxed text-faint">{t('privacy')}</p>

      <button
        type="submit"
        disabled={busy}
        className="min-h-11 w-fit cursor-pointer rounded-card bg-[linear-gradient(135deg,#F0CB7C,#C4923E)] px-6 text-16 font-extrabold text-[#241A05] disabled:cursor-not-allowed disabled:opacity-50"
      >
...
function Field({
  ...
  return (
    <label className="grid gap-1.5">
      <span className="text-14 font-semibold text-text">{label}</span>
      {hint ? (
        <span id={describedBy} className="text-13 text-faint">
          {hint}
        </span>
      ) : null}
      <input
        ...
        className="min-h-11 rounded-card border border-line bg-field px-3.5 text-16 text-text"
      />
      {error ? (
  ...
  return (
    <label className="grid gap-1.5">
      ...
      <select
        ...
        aria-invalid={error ? true : undefined}
        className="min-h-11 cursor-pointer rounded-card border border-line bg-field px-3.5 text-16 text-text"
      >
