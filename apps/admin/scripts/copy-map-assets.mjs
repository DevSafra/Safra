 ...
 *
 * The console needs it for the same reason the customer site and the partner portal do: the
 * landmark registry places a landmark on a MapLibre map with Arabic labels. Each app builds
 * independently, so each provisions its own copy rather than reaching into a sibling's
 * `public/`.
 *
