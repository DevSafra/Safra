...
export default async function middleware(request: NextRequest) {
   ...
   */
  /*
    One derivation, shared with `LandmarkRegistry`, so the policy names exactly what the map
    fetches. The console grew a map on 2026-09-23 — the landmark picker — and a CSP that did
    not know about it produced a blank box with three console errors and a 200 response, which
    is the failure mode `pnpm verify` cannot see. Found by driving the screen, not by reading it.
  */
