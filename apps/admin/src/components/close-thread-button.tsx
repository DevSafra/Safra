  ...
  return (
    <div className="mt-3 flex flex-wrap items-center gap-2 border-t border-line pt-3">
      <button
        ...
        disabled={busy}
        onClick={() => void end()}
        className="inline-flex min-h-10 cursor-pointer items-center rounded-lg border border-line px-3.5 py-1.5 text-13 font-bold text-muted transition-colors hover:border-[rgba(var(--goldA),0.45)] hover:text-gold-read disabled:cursor-not-allowed disabled:opacity-50 lg:min-h-0"
      >
        {busy ? c.closingThread : c.closeThread}
      </button>
      <span className="text-13 text-faint">{c.closeThreadHint}</span>
