  ...
  return (
    <div className="grid gap-4">
      <ConsolePanel>
        ...
        <p className="mt-1.5 max-w-[80ch] text-13 leading-relaxed text-text2">
        ...
        */}
        <p className="mt-3 rounded-lg border border-[rgba(var(--warnA),0.3)] bg-[rgba(var(--warnA),0.07)] px-3 py-2 text-14 leading-relaxed text-warn">
        ...
        <div className="mt-3.5 flex flex-wrap items-end gap-3">
          ...
          */}
          <label className="grid w-full max-w-[24rem] gap-1">
            <span className="text-13 text-faint">{t.sections.settings.filterLabel}</span>
            <div className="flex gap-2">
              ...
              {query ? (
                <button
                  type="button"
                  onClick={() => setQuery('')}
                  className="shrink-0 cursor-pointer rounded-lg border border-line px-3 text-13 text-muted transition-colors hover:border-[rgba(var(--goldA),0.5)] hover:text-gold-read"
                >
      ...
      {shown.map((group) => (
        <ConsolePanel key={group.id} title={group.title}>
          <p className="mb-2 text-13 leading-relaxed text-faint">{group.note}</p>
