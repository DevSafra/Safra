...
export async function ContractsCard({ partnerReference }: { partnerReference?: string }) {
  ...
  return (
    <ConsolePanel title={t.sections.contracts.title}>
      <p className="mb-3 text-13 text-faint">{t.sections.contracts.hint}</p>
      ...
      {result === 'unauthenticated' ? (
        ...
        <p className="text-14 text-faint">{t.sections.contracts.none}</p>
      ) : (
...
function ContractRow({ contract }: { contract: ContractItem }) {
  return (
    <div className="flex flex-wrap items-center gap-3 rounded-card border border-line bg-field px-3.5 py-2.75">
      ...
      <span
        aria-hidden
        className="grid size-8 shrink-0 place-items-center rounded-lg border border-[rgba(var(--goldA),0.3)] bg-[rgba(var(--goldA),0.12)] text-13 font-extrabold text-gold-read"
      >
        PDF
      </span>

      <span className="min-w-0">
        <Ltr className="block truncate text-14 font-bold text-text">
          {contract.fileName}
        </Ltr>
      ...
      <span
        ...
        className="shrink-0 cursor-not-allowed rounded-lg border border-line px-3.5 py-1 text-13 font-bold text-faint"
      >
