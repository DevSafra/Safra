...
export function CancellationPolicyManager({
  ...
  const columns: readonly AdminColumn<CancellationPolicyRow>[] = [
    ...
    },
    {
      ...
      header: c.colTiers,
      render: (row) => (
        <span className="text-13 text-text2">{ladder(row.tiers).join(' · ')}</span>
      ),
    },
    {
      key: 'floor',
  ...
  return (
    <section className="grid gap-3">
      <div className="flex flex-wrap items-baseline gap-2.5">
        <h2 className="text-16 font-extrabold text-gold-read">{c.policiesTitle}</h2>
      ...
      */}
      <p className="rounded-card border border-[rgba(var(--goldA),0.3)] bg-[rgba(var(--goldA),0.06)] p-3 text-13 leading-relaxed text-gold-read">
        {c.policiesNote}
      </p>
  ...
  return (
    ...
    >
      ...
      <div className="grid gap-2 rounded-card border border-line bg-card p-3">
        <p className="text-13 font-bold text-gold-read">{c.tiers}</p>
        <p className="text-14 leading-relaxed text-faint">{c.tiersHint}</p>
        ...
        {tiers.map((tier, index) => (
          ...
          >
            ...
            <button
              ...
              className="inline-flex min-h-10 cursor-pointer items-center rounded-lg border border-line px-3 text-13 text-muted transition-colors hover:border-bad/50 hover:text-bad disabled:cursor-not-allowed disabled:opacity-40 lg:min-h-0 lg:py-1.5"
            >
              ✕
            </button>
          </div>
        ...
        <button
          ...
          className="inline-flex min-h-10 w-fit cursor-pointer items-center rounded-lg border border-line px-3 text-13 text-muted transition-colors hover:border-[rgba(var(--goldA),0.45)] hover:text-gold-read disabled:cursor-not-allowed disabled:opacity-40 lg:min-h-0 lg:py-1.5"
        >
          {c.tierAdd}
        </button>

        {/* Read back in the order the refund service will apply it — see `ladder`. */}
        <p className="text-14 text-text2">{ladder(tiers).join(' · ')}</p>
      </div>
