...
export function StaffInvite({ roles }: { roles: readonly StaffRole[] }) {
  ...
  return (
    <section className="rounded-lg border border-line bg-card p-4">
      <h2 className="text-16 font-extrabold text-gold-read">{t.sections.staff.invite}</h2>
      <p className="mt-1 text-13 leading-relaxed text-faint">
      ...
      >
        ...
        */}
        ...
        <button
          type="submit"
          disabled={busy}
          className="cursor-pointer rounded-lg bg-[linear-gradient(135deg,#F0CB7C,#C4923E)] px-5 py-2.5 text-14 font-extrabold text-[#241A05] disabled:opacity-60"
        >
          {busy ? t.sections.staff.inviteSending : t.sections.staff.inviteSend}
        </button>
      </form>

      <p className="mt-2.5 text-14 text-faint">{t.sections.staff.inviteNote}</p>
    </section>
  );
}
