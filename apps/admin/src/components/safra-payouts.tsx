...
export function SafraPayouts({ payouts }: { readonly payouts: readonly SafraPayout[] }) {
  ...
  const columns: readonly AdminColumn<SafraPayout>[] = [
    {
      ...
      render: (row) => (
        <span className="font-mono text-13 font-bold text-sky">{row.reference}</span>
      ),
    },
    {
      key: 'period',
    ...
    },
    {
      ...
      render: (row) =>
        row.accountLabel ? (
          ...
          <span className="text-13 text-warn">{c.noDestination}</span>
        ),
    },
    {
      key: 'status',
  ...
  return (
    <section className="grid gap-3">
      ...
      <p className="text-13 leading-relaxed text-faint">{c.payoutsNote}</p>

      {error ? (
        <p role="alert" className="text-13 font-semibold text-bad">
          {error}
        </p>
      ) : null}
      ...
      {payouts.some((one) => one.entryGroupId) ? (
        <ul className="grid gap-1">
          {payouts
            ...
            .map((one) => (
              <li key={one.id} className="text-14 text-faint">
...
function Transitions({
  ...
  if (final) return <span className="text-13 text-faint">—</span>;
    ...
    return (
      <span className="flex flex-wrap items-end gap-2">
        ...
        <button
          ...
          className="inline-flex min-h-10 cursor-pointer items-center rounded-lg bg-ok px-3 py-1.5 text-13 font-bold text-bg disabled:cursor-not-allowed disabled:opacity-50 lg:min-h-0"
        >
          {c.markPaid}
        </button>
      </span>
...
function Small({
  ...
  return (
    <button
      ...
      {...{ [marker]: id }}
      onClick={onClick}
      className="inline-flex min-h-10 cursor-pointer items-center rounded-lg border border-line px-2.5 py-1 text-13 text-muted transition-colors hover:border-[rgba(var(--goldA),0.45)] hover:text-gold-read disabled:cursor-not-allowed disabled:opacity-50 lg:min-h-0"
    >
      {label}
    </button>
  );
