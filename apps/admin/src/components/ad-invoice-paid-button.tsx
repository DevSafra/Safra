  ...
  if (!open) {
    return (
      <button
        ...
        className="inline-flex w-fit cursor-pointer items-center justify-center whitespace-nowrap rounded-lg border border-line px-2.5 py-1 text-13 text-muted transition-colors hover:border-[rgba(var(--goldA),0.5)] hover:text-gold-read"
      >
        {c.markPaid}
      </button>
    );
  ...
  return (
    <div className="grid gap-1.5">
      <input
        ...
        aria-label={c.note}
        className="rounded-lg border border-line bg-card px-2 py-1 text-13 text-text placeholder:text-faint"
      />

      {error ? <span className="text-13 font-semibold text-bad">{error}</span> : null}
      ...
      <div className="flex flex-wrap gap-1.5">
        <button
          ...
          onClick={() => void submit()}
          className="cursor-pointer rounded-lg border border-[rgba(var(--goldA),0.4)] px-2.5 py-0.5 text-13 font-bold text-gold-read transition-colors disabled:cursor-not-allowed disabled:opacity-50"
        >
        ...
        <button
          ...
          className="cursor-pointer rounded-lg border border-line px-2.5 py-0.5 text-13 text-muted"
        >
