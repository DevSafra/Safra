  ...
  return (
    <div className="grid gap-1">
      <button
        ...
        disabled={busy}
        onClick={() => void submit()}
        className={`min-h-10 cursor-pointer rounded-lg border px-2.5 py-1 text-13 font-bold transition-colors disabled:opacity-50 lg:min-h-0 ${
          isActive
      ...
      {error ? <span className="text-13 text-bad">{error}</span> : null}
    </div>
  );
}
