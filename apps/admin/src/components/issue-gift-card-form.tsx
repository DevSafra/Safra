  ...
  */
  ...
  const panel = issued ? (
    <div className="grid w-full gap-2 rounded-card border border-[rgba(var(--goldA),0.4)] bg-field p-3.5">
      <h3 className="text-14 font-bold text-gold-read">{c.issuedTitle}</h3>
      {/*
      ...
      <p
        dir="ltr"
        className="select-all rounded-lg border border-line bg-card px-3 py-3 text-center font-mono text-14 leading-relaxed font-bold tracking-[0.1em] break-all text-text sm:text-16 sm:tracking-[0.16em]"
      >
        {issued.code}
      </p>
      <p className="text-13 font-semibold text-bad">{c.issuedCodeOnce}</p>
      {/* Always: the address is required, so a card is never issued without being sent. */}
      <p className="text-13 text-muted">{c.issuedEmailed}</p>
      <div className="flex">
        <button
          type="button"
    ...
    <div className="grid w-full gap-3 rounded-card border border-line bg-field p-3.5">
      <h3 className="text-14 font-bold text-text">{c.issueTitle}</h3>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        <label className="grid gap-1.5 text-13 font-semibold text-muted sm:col-span-2">
          ...
          <input
            ...
            className="field-ltr rounded-lg border border-line bg-card px-3 py-2 text-14 text-text"
          />
        </label>
      </div>
      ...
      */
        ...
        panel === null ? (
          <button
            ...
            className="min-h-10 cursor-pointer rounded-lg border border-[rgba(var(--goldA),0.4)] px-4 py-1.5 text-14 font-extrabold text-gold-read transition-colors hover:bg-[rgba(var(--goldA),0.08)] lg:min-h-0"
          >
            {c.create}
          </button>
        ) : null
