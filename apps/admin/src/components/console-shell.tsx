...
export function NotBuilt({ reason }: { reason: string }) {
  return (
    <section className="rounded-card border border-dashed border-[rgba(var(--goldA),0.35)] bg-card p-6">
      <h2 className="text-16 font-extrabold text-warn">{t.unbuilt.heading}</h2>
      <p className="mt-2.5 max-w-[70ch] text-14 leading-relaxed text-text2">{reason}</p>
      <p className="mt-3 text-14 text-faint">{t.unbuilt.seeRegister}</p>
    </section>
  );
}
...
export function QueueState<T>({
  ...
  */
  ...
  if (state === 'failed') {
    return <p className="text-14 text-bad">{t.dashboard.queueFailed}</p>;
  }

  if (state === 'unauthenticated') {
    return <p className="text-14 text-muted">{t.dashboard.sessionExpired}</p>;
  }

  const rows = Array.isArray(state) ? state : state.items;

  if (rows.length === 0) {
    return <p className="text-14 text-faint">{t.dashboard.nothingWaiting}</p>;
  }
