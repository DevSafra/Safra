...
export function PropertyStarRating({
  ...
  return (
    <div data-star-editor={reference} className="grid gap-3">
      <p className="text-13 leading-relaxed text-faint">{c.starRatingHint}</p>
      ...
      */}
      {value === '' ? (
        <p className="text-14 text-faint">{c.starUnset}</p>
      ) : (
        <StarRating
          value={Number(value)}
          size="md"
      ...
      {saved ? (
        <p className="text-13 font-semibold text-ok">{c.starRatingSaved}</p>
      ) : null}
    </div>
  );
}
