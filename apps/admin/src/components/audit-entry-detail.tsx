  ...
  return (
    <div className="grid gap-4">
      <ConsolePanel title={t.sections.staff.activityWhat}>
        <p className="text-16 font-bold text-text">
        ...
        {entry.reason ? (
          <p className="mt-3 text-14 text-text2">
      ...
      <ConsolePanel title={t.sections.staff.activityChanges}>
        {changes.length === 0 ? (
          ...
          */
          <p className="text-14 text-faint">{t.sections.staff.activityNoChanges}</p>
        ) : (
...
function Row({ term, children }: { term: string; children: React.ReactNode }) {
  return (
    <div className="min-w-0">
      ...
      <dd className="mt-0.5 text-14 text-text">{children}</dd>
    </div>
  );
}
...
function Changes({ changes }: { changes: readonly PayloadChange[] }) {
  ...
  return (
    <dl
      className={`grid gap-x-4 gap-y-1.5 text-14 ${
