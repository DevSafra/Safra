...
export function SafraAccounts({
  ...
  const columns: readonly AdminColumn<SafraAccount>[] = [
    ...
    },
    {
      ...
      render: (row) => (
        <span className="font-mono text-13 text-faint">····{row.last4}</span>
      ),
    },
    {
      key: 'default',
      ...
      render: (row) =>
        row.isDefault ? (
          ...
          <span className="text-13 text-faint">{c.no}</span>
        ),
    },
    {
      key: 'active',
  ...
  return (
    <section className="grid gap-3">
      ...
      <p className="text-13 leading-relaxed text-faint">{c.accountsNote}</p>
      ...
      {open ? (
        ...
        >
          <p className="text-13 font-bold text-gold-read">{c.accountEditTitle}</p>
