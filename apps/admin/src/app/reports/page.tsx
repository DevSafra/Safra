...
function Trend({ card }: { card: ReportCard }) {
  if (card.previous === null) {
    return <p className="mt-1 text-13 text-faint">{t.sections.reports.noPrevious}</p>;
  }
  ...
  if (!Number.isFinite(delta) || Math.abs(delta) < 0.05) {
    return <p className="mt-1 text-13 text-faint">— {t.sections.reports.vsPrevious}</p>;
  }
  ...
  return (
    <p className={`mt-1 text-13 ${good ? 'text-ok' : 'text-bad'}`}>
