  ...
  */
  ...
  return (
    <Shell back={back}>
      ...
      */}
      {query['created'] === '1' ? (
        <p className="rounded-lg border border-ok/30 bg-ok/5 px-4 py-3 text-14 leading-relaxed text-ok">
          {fill(
            query['adopted'] === '1'
      ...
      >
        <p className="mb-3 text-14 leading-relaxed text-muted">
  ...
  return (
    <section className={`rounded-lg border ${tone.border} bg-field/40 p-4`}>
      <div className="flex flex-wrap items-baseline justify-between gap-2">
        ...
        <span className={`text-13 font-semibold ${tone.text}`}>{stateWord}</span>
      </div>

      <p className={`mt-1 text-13 ${tone.text}`}>{note}</p>
