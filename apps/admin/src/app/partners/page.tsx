        ...
        <ConsolePanel title={t.sections.partners.title}>
          ...
          */}
          <div className="mb-3 flex justify-end">
            <Link
              href="/partners/new"
              className="inline-flex min-h-10 items-center rounded-lg border border-ok/40 px-4 py-2 text-14 font-semibold text-ok hover:bg-ok/5 lg:min-h-0"
            >
    ...
    render: (row) => (
      <span className="min-w-0">
        ...
        {row.displayName !== row.legalName ? (
          <span className="block truncate text-13 text-faint">{row.displayName}</span>
        ) : null}
      </span>
    ),
  },
