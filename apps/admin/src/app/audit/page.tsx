    ...
    <ConsoleShell title={t.nav.audit} counts={counts}>
      <ConsolePanel>
        <div className="mb-3 flex flex-wrap items-center gap-2.5">
          ...
          <span className="rounded-full border border-[rgba(var(--badA),0.4)] bg-[rgba(var(--badA),0.12)] px-3 py-0.5 text-13 font-extrabold text-bad">
    ...
    */
    render: (row) => (
      <div className="grid gap-0.5">
        ...
        <Ltr className="text-13 text-faint">{shortDate(row.createdAt)}</Ltr>
      </div>
    ),
  },
  {
    ...
    render: (row) => (
      ...
      */
      <div className="grid gap-0.5">
        ...
        {row.subject?.label ? (
          <span className="text-13 text-text2">{row.subject.label}</span>
        ) : null}
        {row.subject?.reference ? (
          <Ltr className="text-13 text-sky">{row.subject.reference}</Ltr>
        ) : row.subjectId ? (
          <Ltr className="text-13 text-sky">{row.subjectId.slice(0, 8)}</Ltr>
        ) : null}
      </div>
    ),
  },
