        ...
        <div className="grid gap-4">
          ...
          <ConsolePanel title={t.sections.emergency.history}>
            {result.history.length === 0 ? (
              <p className="text-14 text-faint">{t.sections.emergency.never}</p>
            ) : (
              <ul className="grid gap-2.5">
                {result.history.map((mode) => (
                  ...
                  >
                    ...
                    <p className="mt-1.5 text-13 text-text2">{flagSummary(mode)}</p>
                    ...
                    */}
                    {mode.reason ? (
                      <p className="mt-1 text-14 leading-relaxed text-muted">
                        {mode.reason}
                      </p>
                    ) : null}
                  </li>
...
function ActiveBanner({ mode }: { mode: EmergencyMode }) {
  return (
    <div className="flex flex-wrap items-center gap-3 rounded-card border border-bad bg-[rgba(var(--badA),0.12)] px-4 py-3">
      <span className="text-14 font-extrabold text-bad">
