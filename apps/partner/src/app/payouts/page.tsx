      ...
      {payouts === 'unauthenticated' ? (
        ...
        */
        <div className="grid max-w-3xl gap-4">
          ...
          */}
          {suspended ? (
            <p className="rounded-card border border-bad/40 bg-bad/5 px-3.5 py-2.5 text-14 leading-relaxed text-text">
    ...
    <section className="grid gap-2 rounded-card border border-line bg-card p-4.5">
      ...
      */}
      <span className="flex flex-wrap items-baseline justify-between gap-x-3 gap-y-1">
        ...
        <Link
          href="/settings"
          className="inline-flex min-h-10 items-center text-13 font-semibold text-gold-read underline underline-offset-2 lg:min-h-0"
        >
      ...
      */}
      {verified ? (
        ...
        <p className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1.5 rounded-lg border border-warn/40 bg-warn/5 px-3 py-2 text-13 leading-relaxed text-warn">
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
        <h2 className="text-14 font-bold text-text">{t.payouts.fineTitle}</h2>
        <span className="ms-auto text-14 font-bold tabular-nums text-text">
          {t.payouts.fineOutstanding}:{' '}
          {[...totals].map(([code, sum]) => amount(sum.toFixed(2), code)).join(' · ')}
        </span>
      </div>

      <p className="text-13 leading-relaxed text-text2">{t.payouts.fineNote}</p>
      ...
      <ul className="grid gap-1.5">
        {rows.map((row) => (
          <li
            ...
            className="flex flex-wrap items-baseline gap-x-3 gap-y-1 rounded-lg border border-line px-3 py-2 text-13"
          >
...
function Recoveries({ rows }: { readonly rows: readonly PartnerRecovery[] }) {
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
        <h2 className="text-14 font-bold text-text">{t.payouts.recoveryTitle}</h2>
        <span className="ms-auto text-14 font-bold tabular-nums text-text">
      ...
      <p className="text-13 leading-relaxed text-text2">{t.payouts.recoveryNote}</p>
...
function Withheld({
  ...
  */
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
        <h2 className="text-14 font-bold text-warn">{t.payouts.withheldHeading}</h2>
        <span className="ms-auto text-14 font-bold tabular-nums text-warn">
          {t.payouts.withheldTotal}:{' '}
          {[...totals].map(([code, sum]) => amount(sum.toFixed(2), code)).join(' · ')}
        </span>
      </div>

      <p className="text-13 leading-relaxed text-text2">{t.payouts.withheldNote}</p>
      ...
      */}
      {blocked.length > 0 ? (
        <p className="text-13 font-semibold text-warn">
          {blocked.length === 1
      ...
      <ul className="grid gap-2">
        {rows.map((row) => (
          <li
            key={row.reference}
            className="grid gap-1 rounded-lg border border-line2 bg-card p-3 text-13"
          >
            <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
              <Ltr className="font-mono text-14 text-text2">{row.reference}</Ltr>
              <span className="ms-auto font-bold tabular-nums text-text">
            ...
            */}
            {row.responseCount === 0 ? (
              <p className="text-14 font-semibold leading-relaxed text-warn">
...
function Group({
  ...
  return (
    <section className="grid gap-2">
      <h2 className="text-13 font-bold tracking-wide text-faint">
    ...
    >
      <span className="grid gap-1.5">
        <span className="flex flex-wrap items-center gap-x-2.5 gap-y-1.5">
          <Ltr className="text-14 font-bold text-sky">{payout.reference}</Ltr>
          <span
            className={`rounded-full border px-2.5 py-0.5 text-14 font-bold ${TONES[statusTone(payout.status)]}`}
          >
        ...
        {payout.holdReason ? (
          <span className="text-13 leading-relaxed text-warn">
      ...
      <span className="grid gap-0.5 sm:justify-items-end sm:text-end">
        <span className="text-18 font-extrabold text-gold-read">
