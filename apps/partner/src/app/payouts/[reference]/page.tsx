    ...
    <Shell title={t.payouts.title} partnerName={name} active="payouts">
      <div className="grid gap-4">
        ...
        <header className="flex flex-wrap items-center gap-3">
          <Ltr className="text-16 font-bold text-sky">{payout.reference}</Ltr>
          <span
        ...
        <section>
          ...
          {coveredFailed ? (
            ...
            <p className="text-14 text-faint">{t.payouts.noBookings}</p>
          ) : (
            <ul className="grid gap-2">
              ...
              */}
              {covered.map((booking) => (
                ...
                >
                  <Ltr className="text-14 font-semibold text-sky">
                    {booking.bookingReference}
                  </Ltr>
                  <span className="text-13 text-muted">{booking.property ?? ''}</span>
                  <Ltr className="text-14 tabular-nums text-muted">
                    {booking.checkIn} ← {booking.checkOut}
                  </Ltr>
                  <span className="ms-auto grid justify-items-end gap-0.5">
                    <Ltr className="text-14 font-bold text-gold-read">
        ...
        */}
        {covered.some((one) => Number(one.refunded) > 0) ? (
          <p className="rounded-lg border border-line bg-field px-3 py-2 text-13 leading-relaxed text-muted">
...
function Row({
  ...
  return (
    <div className="flex items-center justify-between gap-3 rounded-card border border-line bg-card px-3.5 py-2.5">
      ...
      <span
        className={`text-14 ${strong ? 'font-extrabold text-gold-read' : 'text-text'}`}
      >
