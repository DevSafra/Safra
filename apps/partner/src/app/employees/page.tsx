...
export default async function EmployeesPage({
  ...
  */
  ...
  return shell(
    <>
      <p className="text-14 leading-relaxed text-muted">{t.employees.intro}</p>
    ...
    <div className="grid gap-3 rounded-card border border-line bg-card p-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="grid gap-0.5">
          ...
          */}
          <Ltr className="text-14 text-muted">{employee.email}</Ltr>
        </div>

        <div className="flex flex-wrap items-center gap-2">
          <span
            className={`rounded-full border px-2.5 py-0.5 text-14 font-bold ${TONES[statusTone(employee.status)]}`}
          >
          ...
          {progress ? (
            <span className={`text-13 ${progress.tone}`}>{progress.label}</span>
          ) : null}
        </div>
      </div>
