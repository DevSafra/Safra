...
export default async function ViolationsPage({
  ...
  return shell(
    <>
      <p className="text-14 leading-relaxed text-muted">{t.violations.intro}</p>
  ...
  return (
    <div className="grid gap-1.5 rounded-card border border-line bg-card p-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <Ltr className="text-13 text-faint">{violation.createdAt}</Ltr>
        {/* The affordance, so a card that opens looks like one. */}
        <span className="text-13 text-gold-read">{t.violations.open} ←</span>
      </div>
