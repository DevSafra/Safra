...
export default async function CouponsPage() {
  ...
  */
  ...
  return shell(
    <>
      <p className="text-14 leading-relaxed text-muted">{t.coupons.intro}</p>
...
function Section({
  ...
  return (
    <section>
      <h2 className="mb-2 text-16 font-extrabold text-gold-read">{title}</h2>

      {rows.length === 0 ? (
        <p className="text-14 text-faint">{empty}</p>
      ) : (
          ...
          {rows.map((coupon) => (
            ...
            >
              <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
                <span className="font-mono text-14 font-bold text-text">
                  {coupon.code}
                </span>
                <span className="text-14 font-semibold text-gold-read">
                  {discountOf(coupon)}
                </span>
                {coupon.expired ? (
                  <span className="rounded-full border border-line px-2 py-0.5 text-13 text-faint">
                    {t.coupons.expired}
                  </span>
                ) : null}
              </div>
