...
export default async function SupportPage() {
  ...
  return (
    ...
    >
      <p className="text-14 text-muted">{t.support.intro}</p>

      <section className="mt-4 rounded-card border border-line bg-card p-4">
        <h2 className="font-display text-18 text-text">{t.support.openTitle}</h2>
        <div className="mt-3">
          <SupportForm />
        </div>
      </section>
      ...
      <section className="mt-6">
        <h2 className="font-display text-18 text-text">{t.support.mineTitle}</h2>

        {tickets === 'failed' || tickets === 'unauthenticated' ? (
          <p className="mt-3 text-14 text-bad">{t.dashboard.loadFailed}</p>
        ) : tickets.items.length === 0 ? (
          <p className="mt-3 rounded-card border border-line bg-card p-5 text-center text-14 text-muted">
            {t.support.none}
          </p>
        ) : (
              ...
              <li key={ticket.reference}>
                ...
                >
                  <span className="min-w-0">
                    <Ltr className="block text-14 text-text">{ticket.reference}</Ltr>
                    {ticket.lastMessage ? (
                      <span className="mt-1 block truncate text-14 text-muted">
                        {ticket.lastMessage}
                      </span>
                    ) : null}
