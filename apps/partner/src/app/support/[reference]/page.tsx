...
export default async function SupportThreadPage({
  ...
  return (
    ...
    >
      <Link
        href="/support"
        className="inline-flex min-h-10 w-fit items-center gap-2 rounded-lg border border-line px-4 text-14 text-muted transition-colors hover:border-[rgba(var(--goldA),0.4)] hover:text-gold-read lg:min-h-0 lg:py-2"
      >
      ...
      <section className="mt-4 rounded-card border border-line bg-card p-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <Ltr className="text-14 text-text">{thread.reference}</Ltr>
          <span
            className={`rounded-full border px-2 py-0.5 text-14 ${
              thread.closed
        ...
        <ol className="mt-4 space-y-2">
          {thread.messages.map((message) => (
            ...
            >
              ...
              <p className="mt-2 text-14 leading-relaxed whitespace-pre-wrap text-text">
                {/*
              ...
              {message.redactedCount > 0 ? (
                <p className="mt-2 text-14 text-warn">
      ...
      {thread.closed ? (
        <p className="mt-4 rounded-card border border-line bg-card p-4 text-14 text-muted">
