...
export default async function UnitCalendarPage({
  ...
  if (property.units.length === 0) {
    return shell(<p className="text-14 text-faint">{t.unitCalendar.noUnits}</p>);
  }
  ...
  return shell(
    <>
        ...
        <nav aria-label={t.unitCalendar.unit} className="flex flex-wrap gap-1.5">
          {property.units.map((candidate) => (
            <Link
              ...
              className={`inline-flex min-h-10 items-center rounded-lg border px-3 text-13 lg:min-h-0 lg:py-1.5 ${
      ...
      <nav aria-label={t.unitCalendar.month} className="flex items-center gap-2">
        ...
        <span className="text-14 font-bold text-text" dir="ltr">
          {month}
        </span>
        <Link
          href={link(unit.id, shift(month, 1))}
