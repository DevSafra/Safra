    ...
    <>
      <section className="rounded-card border border-line bg-card p-5">
        <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
          <span className="text-16 font-bold text-sky">
            <Ltr>{dispute.reference}</Ltr>
          </span>
          <span className="text-13 text-muted">{disputeStatus(dispute.status)}</span>
          <span className="text-13 text-faint">{disputeKind(dispute.kind)}</span>
          {dispute.frozenAmount !== null && dispute.currencyCode !== null ? (
            <span className="ms-auto text-16 font-extrabold text-warn">
      ...
      <section className="rounded-card border border-line bg-card p-5">
        <h2 className="text-14 font-bold text-text">{t.disputes.allegation}</h2>
        <p className="mt-1.5 text-14 font-semibold text-text">
          {renderRedactions(dispute.title, 'ar')}
        </p>
        <p className="mt-2 text-14 leading-relaxed whitespace-pre-line text-muted">
          {dispute.description
      ...
      */}
      <section className="rounded-card border border-line bg-card p-5">
        <h2 className="text-14 font-bold text-text">{t.disputes.decision}</h2>
        <p className="mt-1.5 text-14 leading-relaxed whitespace-pre-line text-muted">
          {dispute.resolution
      ...
      <section className="rounded-card border border-line bg-card p-5">
        <h2 className="text-14 font-bold text-text">{t.disputes.yourResponses}</h2>

        {dispute.responses.length === 0 ? (
          <p className="mt-1.5 text-14 text-faint">{t.disputes.noResponses}</p>
        ) : (
          <ul className="mt-2 grid gap-2">
            {dispute.responses.map((response) => (
              ...
              >
                <p className="text-14 leading-relaxed whitespace-pre-line text-text">
        ...
        <div className="mt-4 border-t border-line pt-4">
          {closed ? (
            <p className="text-14 text-faint">{t.disputes.respondClosed}</p>
          ) : (
            <>
              <h3 className="text-14 font-bold text-text">{t.disputes.respondHeading}</h3>
              <p className="mt-1 mb-3 text-13 leading-relaxed text-faint">
      ...
      <section className="rounded-card border border-line bg-card p-5">
        <h2 className="text-14 font-bold text-text">{t.disputes.evidence}</h2>
        ...
        */}
        {dispute.withheldEvidenceCount > 0 ? (
          <p className="mt-3 rounded-lg border border-line bg-field px-3 py-2 text-13 leading-relaxed text-faint">
