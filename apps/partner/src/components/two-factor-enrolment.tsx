...
export function TwoFactorEnrolment() {
  ...
  return (
    <div className="grid gap-4">
      ...
      <ol className="grid gap-2 text-14 text-muted">
      ...
      <div>
        <p className="text-13 text-faint">{t.twoFactor.setupKey}</p>
        <p
          dir="ltr"
      ...
      <form onSubmit={(event) => void confirm(event)} className="grid gap-3">
        <label htmlFor="code" className="text-14 text-muted">
      ...
      <form action="/api/auth/logout" method="post">
        <button
          type="submit"
          className="min-h-10 w-full cursor-pointer rounded-lg border border-line px-4 py-2 text-14 text-faint hover:text-muted"
        >
          {t.twoFactor.signOut}
        </button>
      </form>
