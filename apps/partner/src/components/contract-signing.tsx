...
export function ContractSigning({
  ...
  if (status === 'active') {
    return <p className="text-13 text-ok">{t.contracts.signDone}</p>;
  }

  if (status === 'draft') {
    return <p className="text-13 text-faint">{t.contracts.signWaitingSafra}</p>;
  }
  ...
  return (
    ...
    >
      <p className="text-14 font-bold text-gold-read">{t.contracts.signTitle}</p>
      <p className="text-13 leading-relaxed text-text2">{t.contracts.signSteps}</p>

      <label className="grid gap-1">
        <span className="text-13 text-muted">{t.contracts.signFile}</span>
        <input
          type="file"
          accept="application/pdf"
          required
          onChange={(event) => setFile(event.target.files?.[0] ?? null)}
          className="min-h-10 cursor-pointer rounded-lg border border-line bg-card px-3 py-2 text-14 text-text lg:min-h-0"
        />
      </label>
      ...
      <button
        type="submit"
        disabled={busy || !file}
        className="min-h-10 w-fit cursor-pointer rounded-lg bg-[linear-gradient(135deg,#F0CB7C,#C4923E)] px-5 text-14 font-extrabold text-[#241A05] disabled:cursor-not-allowed disabled:opacity-50 lg:min-h-0 lg:py-2"
      >
