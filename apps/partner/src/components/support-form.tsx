  ...
  return (
    <form onSubmit={(event) => void submit(event)} className="grid gap-3">
      ...
      <label className="grid gap-1">
        ...
        />
        <span className="text-13 text-faint">{t.support.bodyHint}</span>
      </label>

      <button
        type="submit"
        disabled={busy}
        className="min-h-10 w-fit cursor-pointer rounded-lg btn-gold px-5 text-14 font-semibold text-ink disabled:cursor-not-allowed disabled:opacity-60 lg:min-h-0 lg:py-2"
      >
        {busy
