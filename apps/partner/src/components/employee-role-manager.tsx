  ...
  return (
    ...
    >
      ...
      <label className="grid gap-1 text-14 text-muted">
        ...
        />
        <span className="text-13 text-faint">{t.employeeRoles.nameHint}</span>
      </label>

      <fieldset className="grid gap-2">
        <legend className="text-14 text-muted">
        ...
        */}
        {groupPermissions(capabilities).map(({ group, permissions }) => (
          <div key={group} className="grid gap-1.5">
            <p className="text-13 font-semibold text-faint">{groupLabel(group)}</p>

            <div className="grid gap-1.5 sm:grid-cols-2">
              {permissions.map((permission) => (
                <label
                  key={permission}
                  className="flex cursor-pointer items-center gap-2 text-14 text-text"
                >
                  <input
                    type="checkbox"
      ...
      */}
      ...
      openableSections(chosen, PARTNER_SECTION_PERMISSIONS).length === 0 ? (
        <p className="rounded-lg border border-warn/40 bg-warn/10 px-3 py-2 text-14 leading-relaxed text-warn">
              ...
              <li key={role.id}>
                <div className="grid gap-2 rounded-card border border-line bg-card p-4">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    */}
                    {role.employeeCount > 0 ? (
                      <span className="text-13 text-faint">{t.employeeRoles.inUse}</span>
                    ) : (
                      <button
                        type="button"
                        disabled={busyId !== null}
