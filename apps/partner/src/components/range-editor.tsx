...
export function RangeEditor({
  ...
  return (
    ...
    >
      <p className="text-14 font-bold text-text">{t.unitCalendar.editorTitle}</p>

      {message ? (
        <p
          role="alert"
          className={`rounded-lg border p-3 text-14 ${
            message.kind === 'ok'
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <label className="grid gap-1">
          <span className="text-13 text-muted">{t.unitCalendar.from}</span>
          <input
            type="date"
            required
            value={range.from}
        ...
        <label className="grid gap-1">
          <span className="text-13 text-muted">{t.unitCalendar.to}</span>
          <input
            type="date"
            required
            value={range.to}
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <label className="grid gap-1">
          <span className="text-13 text-muted">{t.unitCalendar.status}</span>
          <select
            value={status}
        ...
        <label className="grid gap-1">
          <span className="text-13 text-muted">{t.unitCalendar.price}</span>
          {/*
          ...
          */}
          ...
          <button
            ...
            className={`min-h-10 w-fit cursor-pointer rounded-lg border px-2.5 py-1 text-13 lg:min-h-0 ${
      ...
      <label className="grid gap-1">
        <span className="text-13 text-muted">{t.unitCalendar.note}</span>
        <input
          value={note}
          maxLength={500}
          ...
          className="min-h-10 rounded-lg border border-line bg-field px-3 py-2 text-14 text-text lg:min-h-0"
        />
        <span className="text-13 text-faint">{t.unitCalendar.noteHint}</span>
      </label>

      <p className="text-13 leading-relaxed text-faint">{t.unitCalendar.bookedWarning}</p>
