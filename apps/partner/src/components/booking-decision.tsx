...
export function BookingDecision({
    ...
    return (
      ...
      */
      <p
        data-decided
        role="status"
        className={`flex flex-wrap items-baseline gap-x-2 gap-y-0.5 rounded-lg border p-3 text-14 ${
      ...
      >
        ...
        <div className="flex flex-wrap gap-2">
          <button
            type="submit"
            disabled={busy}
            className="min-h-10 cursor-pointer rounded-lg border border-bad/50 px-4 py-2 text-14 font-bold text-bad transition-transform duration-150 ease-out-strong active:scale-[0.97] disabled:cursor-not-allowed disabled:opacity-60 motion-reduce:transition-none lg:min-h-0"
          >
          ...
          <button
            ...
            className="min-h-10 cursor-pointer rounded-lg border border-line px-4 py-2 text-14 text-muted transition-transform duration-150 ease-out-strong active:scale-[0.97] motion-reduce:transition-none lg:min-h-0"
          >
            {d.cancel}
          </button>
        </div>
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      {error ? (
        <p role="alert" className="w-full text-13 text-bad">
          {error}
        </p>
      ) : null}
      ...
      <button
        ...
        disabled={busy}
        onClick={() => void accept()}
        className="min-h-10 cursor-pointer rounded-lg border-none bg-[linear-gradient(135deg,#8FD9A8,#4F9E6B)] px-4 py-2 text-14 font-extrabold text-[#0A2013] transition-transform duration-150 ease-out-strong active:scale-[0.97] disabled:cursor-not-allowed disabled:opacity-60 motion-reduce:transition-none lg:min-h-0"
      >
      ...
      <button
        ...
        className="min-h-10 cursor-pointer rounded-lg border border-bad/50 bg-transparent px-4 py-2 text-14 font-bold text-bad transition-transform duration-150 ease-out-strong active:scale-[0.97] disabled:cursor-not-allowed disabled:opacity-60 motion-reduce:transition-none lg:min-h-0"
      >
        {d.reject}
      </button>
