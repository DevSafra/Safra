...
export function PropertyAmenities({
  ...
  return (
    <section className="grid gap-2.5">
      <h3 className="text-14 font-bold text-text">{c.propertyAmenitiesLabel}</h3>
      <p className="text-13 text-faint">{c.propertyAmenitiesHint}</p>
      ...
      <span>
        <button
          ...
          disabled={busy || !dirty}
          onClick={() => void save()}
          className="inline-flex min-h-10 w-fit cursor-pointer items-center rounded-lg btn-gold px-4 py-2 text-14 font-bold disabled:cursor-not-allowed disabled:opacity-50 lg:min-h-0"
        >
