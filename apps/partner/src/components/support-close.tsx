  ...
  return (
    <form onSubmit={(event) => void submit(event)} className="grid gap-2">
      ...
      <button
        type="submit"
        disabled={busy}
        className="min-h-10 w-fit cursor-pointer rounded-lg border border-line px-5 text-14 text-muted transition-colors hover:border-[rgba(var(--goldA),0.4)] hover:text-gold-read disabled:cursor-not-allowed disabled:opacity-60 lg:min-h-0 lg:py-2"
      >
        {busy ? t.support.closeSubmitting : t.support.closeLabel}
      </button>

      <span className="text-13 text-faint">{t.support.closeHint}</span>
    </form>
  );
}
