...
export function DisputeEvidence({
  ...
  */
  ...
  return (
    <>
      {evidence.length === 0 ? (
        <p className="mt-1.5 text-14 text-faint">{c.evidenceNone}</p>
      ) : (
      ...
      */}
          ...
          {evidence
            ...
            .map((one) => (
              <li
                ...
                className="grid h-20 w-28 place-items-center rounded-lg border border-dashed border-line px-2 text-center text-14 leading-tight text-faint"
              >
      ...
      {closed ? (
        <p className="mt-3 text-13 text-faint">{c.evidenceClosedNote}</p>
      ) : (
        <div className="mt-3 border-t border-line pt-3">
          <p className="mb-2 text-13 leading-relaxed text-faint">{c.evidenceHint}</p>

          <button
            type="button"
            disabled={busy}
            onClick={() => file.current?.click()}
            className="min-h-10 cursor-pointer rounded-lg border border-line bg-field px-4 py-2 text-14 font-bold text-text transition-colors duration-150 ease-out-strong hover:border-gold hover:text-gold-read disabled:cursor-not-allowed disabled:opacity-60 motion-reduce:transition-none lg:min-h-0"
          >
