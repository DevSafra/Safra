    ...
    <form className="grid gap-4" onSubmit={(event) => void save(event)}>
      ...
      */}
      ...
      <fieldset className="grid gap-2">
        <legend className="pb-1 text-13 text-muted">
          ...
          {TRIP_ATTRIBUTES.map((attribute) => {
            ...
            return (
              <button
                ...
                className={`min-h-10 cursor-pointer rounded-full border px-3 py-1 text-13 lg:min-h-0 ${
        ...
        {attributes.length > 4 ? (
          <p className="text-13 text-bad">{t.properties.attributesTooMany}</p>
        ) : null}
      </fieldset>
