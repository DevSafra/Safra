...
export function AmenityPicker({
  ...
  if (amenities.length === 0) {
    return <p className="text-13 text-faint">{c.amenitiesNone}</p>;
  }
  ...
  return (
    <fieldset className="grid gap-2.5" data-amenity-picker={idPrefix}>
      <legend className="text-13 text-muted">{c.amenities}</legend>
      ...
      */}
      <p
        ...
        className={`text-13 leading-relaxed ${selected.length > 0 ? 'text-text2' : 'text-faint'}`}
      >
        {selected.length > 0
          ? amenities
      ...
      <p className="text-14 leading-relaxed text-faint">{c.amenitiesHint}</p>

      {[...groups.entries()].map(([category, list]) => (
        <div key={category} className="grid gap-1.5">
          <p className="text-14 font-bold text-faint">{groupLabel(category)}</p>
          ...
          <div className="flex flex-wrap gap-x-4 gap-y-1">
            {list.map((amenity) => (
              ...
              */
              <label
                key={amenity.code}
                className="flex cursor-pointer items-center gap-2 text-13 text-text2"
              >
                <input
                  type="checkbox"
