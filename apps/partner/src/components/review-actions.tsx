      ...
      >
        ...
        <div className="flex flex-wrap gap-2">
          <button
            type="submit"
            disabled={busy}
            className="min-h-10 cursor-pointer rounded-lg border border-gold px-4 py-1.5 text-13 font-bold text-gold-read disabled:cursor-not-allowed disabled:opacity-60 lg:min-h-0"
          >
            {busy
              ? t.reviews.working
              : isReply
    ...
    <div className="mt-2.5 flex flex-wrap items-center gap-2">
      ...
      {hasReply ? null : (
        <button
          ...
          className="min-h-10 cursor-pointer rounded-lg border border-gold px-4 py-1.5 text-13 font-bold text-gold-read lg:min-h-0"
        >
          {t.reviews.reply}
        </button>
      )}
      ...
      {reportNote ? <span className="text-13 text-faint">{reportNote}</span> : null}
    </div>
  );
}
