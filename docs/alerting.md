...
| `safra_fx_rate_age_seconds{currency}` + `safra_fx_currencies_without_rate`  | 18, 18b | **-1 means no rate exists at all.** A wallet holds one currency for ever, so a credit arriving in another converts through SYP; with no rate the conversion REFUSES, and a compensation, a wallet refund or a cross-currency gift card cannot complete. SYP is excluded — it is the quote side, and a series nobody can ever clear is one somebody mutes |
...
groups:
  - name: safra
    rules:
      ...
      # 18 — a currency SAFRA prices in cannot be converted.
      #
      # Not a staleness alert dressed up: -1 is the state where no rate has EVER been set, and on
      # 2026-08-26 that was true of EUR, JOD and LBP while 512 wallets were denominated in EUR.
      # `FxRateService` refuses rather than defaulting to 1, which is correct and is exactly why
      # the absence needs to be visible somewhere other than the exception it throws at whoever
      # happened to try.
      - alert: FxRateMissing
        expr: safra_fx_rate_age_seconds == -1
        for: 15m
        labels: { severity: page }

      # 18b — the rate exists and has gone stale.
      #
      # `STALE_AFTER_HOURS` is 48 in FxRateService, which warns and keeps pricing: refusing on an
      # old rate would take bookings down on a schedule nobody agreed to. So the decision to act
      # is an operational one, and this is where it gets made rather than in the service.
      - alert: FxRateStale
        expr: safra_fx_rate_age_seconds > 172800
        for: 1h
        labels: { severity: ticket }
