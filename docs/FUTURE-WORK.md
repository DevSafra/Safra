...
| **Ad creative images**                          | `ad_campaigns.image_path` exists and nothing writes it, so `PartnerAds` renders text only. Deliberate: an `<img>` built from a free-text path nobody validates means the first thing to write that column decides what SAFRA's pages fetch. Needs the upload path property images already have — variants, `file_key`, a size cap — before the render can be trusted. Campaigns work without it: a headline and an advertiser name are a complete ad. |
...
### `pnpm db:testbed` cannot re-run once a refund has reached the ledger

Found 2026-08-27. The script deletes and recreates its fixtures, keyed on the fixture emails, and its
own docblock calls that idempotent. It is not any more:
...
`ledger_entries.refund_id` references `refunds`, and `ledger_entries` is append-only by trigger — so
the rows blocking the delete cannot be removed OR nulled. **The delete-and-recreate model is
incompatible with the append-only ledger**, and this is not a missing `CASCADE`: cascading would
delete accounting rows, which is the one thing the ledger exists to prevent.

**Why it matters beyond the script.** It is the documented remedy for e2e fixture exhaustion, and
several specs consume a fixture per run:

- `customer-gifts.spec.ts` opens one customer dispute per run and fails when every
  (booking × reason) pair on the fixture account has a live one — **32 spent as of 2026-08-27**, and
  it now has nowhere to go;
- `booking-actions.spec.ts` consumes one `pending_payment` booking per lifecycle run, and skips
  rather than failing when there is none left;
- `enforcement.spec.ts` raises one violation per run.

So the suite degrades with every run and the reset no longer works. **Not fixed — it needs a
decision**, because both plausible answers change the testbed's contract:

1. **Preserve what the ledger references.** Reseed only the bookings whose refunds have no ledger
   legs, and REUSE the rest. Keeps history, but the fixture stops being a known clean state, which
   is most of its value.
2. **A fresh identity per reseed.** Key the fixtures to a run-scoped email rather than a fixed one,
   and leave every previous generation in place. Always clean, and the database grows for ever on a
   developer's machine — acceptable there, and `db:reset-dev` remains the real reset.

Option 2 is the smaller change and the one that matches what the script is FOR. Neither should be
chosen without Bashar, because the fixture emails are what several specs and the seed both key on.
