...
| 196 | The EUR→SYP rate — none is configured   | Wallet compensation in EUR       | 512 of 11,801 wallets are EUR. `FxRateService` refuses rather than defaulting to 1, so a dispute compensation paid to one of those customers now fails with «التسعير غير متاح مؤقتًا» instead of corrupting the balance. Loud, and still a break for those customers until a rate is set via `POST /admin/fx-rates`. The RATE is a business decision |
