...
| 196 | Rates to SYP for EUR, JOD and LBP       | Every cross-currency wallet movement | Three of five active currencies hold NO rate; USD's is 15 days old (`STALE_AFTER_HOURS` is 48). A wallet holds one currency for ever, so a credit arriving in another converts through SYP and REFUSES without a rate — 512 of 11,801 wallets are EUR. Engineering is done: validation refuses rather than defaulting to 1, `safra_fx_rate_age_seconds` reports -1 per currency, and alerts 18/18b are written. **The RATES themselves are a business number nobody but Bashar can set** — `POST /admin/fx-rates` |
