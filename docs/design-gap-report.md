...
| المخالفات description       | **Complete, and was NOT twice.** A `description` column fixed hand-raised violations only; the SLA sweep and the rejection path write none, so the FINED rows had no explanation. A sentence per kind is resolved at render time — never stored, so it stays translatable                     |
| المخالفات detail page       | **New.** A page per violation: what happened, when, which booking, the warning, the fine and its reason, the waiver. Scoped in the WHERE clause — another business's violation answers as one that does not exist                                                                             |
