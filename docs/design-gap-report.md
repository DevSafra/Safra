...
| لوحة التحكم §7.1 | KPIs, calendar, activity, payout line                           | **Skeleton only.** One KPI (listing count). No calendar, no activity, no payout line. The payout line's blocker is gone — the ledger shipped in `a84a67d` — so what remains is a read and a component, not a decision.   |
| المستحقات        | — (not drawn in §7)                                             | **Backend only.** `GET /partner/payouts` and its items endpoint answer, scoped to the signed-in partner by `PAYOUT_READ_OWN`. There is no page. A partner cannot see a payout in the product.                            |
...
| No partner payout UI, staff or partner side  | An operable payout workflow, and the §7.1 dashboard line                                         | The API answers; a person cannot use it. Releasing a transfer currently requires `curl`. The dashboard line must read a `scheduled` payout and stay ABSENT when there is none — summing `partner_payable_amount` into a sentence about a transfer remains forbidden. |
...
| Deviation                                                                   | Reason                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| --------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| No 2FA gate, unlike the console                                             | The API asks a partner for no second factor. A gate would lock every partner out of an app they cannot enter, rather than raising anybody's security. `hasTwoFactor` is already in `@safra/session` for when partner 2FA exists. **No longer a deviation anybody should accept:** Bashar confirmed partner 2FA as intended scope on 2026-08-06, and now that the payout ledger has shipped a partner account is a view onto money owed. Tracked as `O-partner-4`. |
