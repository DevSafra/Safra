...
| عقاراتي §7.2      | header, add form, listing cards                                 | **Complete except image slots.** Cards plus the add form: the auto-fit field grid, the shared صفات الرحلة chip group, «إرسال للمراجعة» and the P-002 note. Creating a listing also opens its units, in one transaction. The three image slots are absent and the form says why — an image uploads against a property that already exists.        |
