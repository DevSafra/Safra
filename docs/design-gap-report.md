...
| عقاراتي §7.2      | header, add form, listing cards                                 | **Complete.** Cards, the add form with the shared صفات الرحلة chips and the P-002 note, and a media manager on الصور: upload, reorder, set cover, alt text, archive. The add form's three image slots remain absent by design — an image uploads against a property that already exists, and the form says so.                                   |
...
| Gap                                                                      | Blocks                                                             | Note                                                                                                                                                                                                                                                                                                            |
| ------------------------------------------------------------------------ | ------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ~~No `reviews` table, API or aggregate~~ — **shipped 2026-08-07**        | Public display on the property page                                | Schema, API, partner UI, staff moderation AND the customer write flow all exist. `properties.rating` is a real aggregate maintained by trigger over published reviews. What remains is showing reviews to visitors on `/property/[slug]`. See `O-partner-1`.                                                    |
...
| ~~No image serving story confirmed end to end~~ — **shipped 2026-08-07** | Nothing                                                            | Upload, list, reorder, cover, alt text and archive all exist and were exercised against the running API with a real JPEG. EXIF including GPS is stripped by re-encoding, verified directly. A partial unique index guarantees one cover per property. See `O-media-1`.                                          |
